...
---Send keys.
---@param keys ccc.kit.Vim.Keymap.KeysSpecifier|ccc.kit.Vim.Keymap.KeysSpecifier[]
...
function Keymap.send(keys, no_insert)
  ...
  return Async.new(function(resolve, _)
    ...
    local callback = Keymap.termcodes(('<Cmd>lua require("ccc.kit.Vim.Keymap")._resolve(%s)<CR>'):format(unique_id))
    if no_insert then
...
function Keymap.to_sendable(callback)
  ...
  return Keymap.termcodes(('<Cmd>lua require("ccc.kit.Vim.Keymap")._resolve(%s)<CR>'):format(unique_id))
end
